...
$sql = "SELECT * FROM boardgames ORDER BY owner ASC, updated_at ASC LIMIT :current_page, :record_per_page";
...
<?= template_header('Jogos a expirar') ?>
<div class="d-flex flex-column bd-highlight mb-3 bg-list" style="flex: 1">
    ...
    <?php foreach ($boardgames as $bg) : ?>
        <?php
            ...
            if ($diffDays > 53 && $diffDays < 60)
                $addedClass = 'd-inline-flex';
            else
                $addedClass = 'd-none';
            $daysToExpire = 60 - $diffDays;
        ?>
        <div class="d-inline-flex justify-content-start align-items-center bg-item <?= $addedClass ?>">
            <div class="bg-fields" style="flex: 1.5">
                <b><?= ucwords($bg['name'])?></b>
                <span>- <?= ucwords($bg['owner'])?></span>
                <b>- <?= $daysToExpire." " ?><?= ngettext("dia", "dias", $daysToExpire) ?> para expirar</b>
            </div>
        </div>
    <?php endforeach; ?>
</div>
